...
TomeWeaver is the **first canonical reference implementation of the Language State Machine (LSM) framework (MMG-LSM-1.0)**. 
...
TomeWeaver’s design is described in **The Language State Machine (LSM) Architecture** — a reference framework for stateful AI narrative orchestration (Protocol `MMG-LSM-1.0`).

- **[Read the PDF](docs/article/MMG-LSM-1.0%20-%20The%20Language%20State%20Machine.pdf)** — formatted systems specification
...
1. **Open Latest Releases** — go to **[Latest Releases](https://github.com/MeaningfulnessMediaGroup/TomeWeaver/releases/latest)** on GitHub.
2. **Download the build for your OS** — under *Assets*, pick the file that matches your platform (version tag is in the filename):
  - **Windows:** `TomeWeaver_Windows_<version>.exe`
  - **macOS:** `TomeWeaver_MacOS_<version>`
  - **Linux:** `TomeWeaver_Linux_<version>`
3. **Prepare a folder** — create a dedicated folder and put the executable inside. On first run, TomeWeaver writes `configs/` and your adventure library next to the binary; keeping everything in one folder makes backups easy.
4. **Launch TomeWeaver**
  - **Windows** — double-click the `.exe`.
  - **macOS / Linux** — from that folder in a terminal:
    On macOS, if Gatekeeper blocks the app, right-click the binary → **Open** once to approve it.
...
1. **Get the Source Code** — clone the repository, or download the `.zip` from **[Latest Releases](https://github.com/MeaningfulnessMediaGroup/TomeWeaver/releases/latest)** and extract it.
  ```cmd
    git clone https://github.com/MeaningfulnessMediaGroup/TomeWeaver.git
    cd TomeWeaver
  ```
2. **Run the Installer** — from the repo root, double-click `setup.bat` or run:
  ```cmd
    setup.bat
  ```
    This creates `venv\` and installs GUI dependencies.
3. **Launch the Engine** — double-click `Start_TomeWeaver.bat` or run:
  ```cmd
    Start_TomeWeaver.bat
  ```
4. **(Optional) Run Automated Tests** — double-click `Run_Tests.bat` or run:
  ```cmd
    Run_Tests.bat
  ```
    Executes the headless pytest suite against the engine core.
...
TomeWeaver is a feature-rich application. Please refer to our dedicated guides for detailed instructions on using the UI and configuring your worlds:

- 🖼️ **[The UI Walkthrough (docs/README.md)](docs/README.md)** - A visual guide to the Library Dashboard, Story Timeline, and Editors.
- 📄 **[LSM Architecture Article (PDF)](docs/article/MMG-LSM-1.0%20-%20The%20Language%20State%20Machine.pdf)** - Systems specification; [LaTeX source](docs/article/main.tex).
...
We welcome contributions! Whether it's improving the "Fortress" sanitizer, adding new export formats, sharing Adventure Cartridges, or **minor fixes to the LSM article LaTeX** (see [PDF](docs/article/MMG-LSM-1.0%20-%20The%20Language%20State%20Machine.pdf)):
